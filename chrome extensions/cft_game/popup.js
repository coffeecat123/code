...
_clr.onchange=()=>{
    let __a=_clr.value;
    chrome.storage.sync.set({bk: __a});
    chrome.storage.sync.get(["bk"], (e) => {
      console.log(e.bk);
    });
    do_something(()=>{
    ...
    document.body.style.background=__a;
    if((parseInt(_clr.value.substring(1,3),16)+parseInt(_clr.value.substring(3,5),16)+parseInt(_clr.value.substring(5,7),16))/3>128){
        root.style.cssText+='--txt-color: #000;';
    }
    else{
...
ipt.addEventListener("change",()=>{
  chrome.storage.sync.set({color: ipt.value});
  do_something(()=>{
    chrome.storage.sync.get(["color"], (e) => {
...
async function do_something(something){
